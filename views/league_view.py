...
class LeagueView:
    def __init__(self, screen):
        ...
        self.menu_options = ["Back to Main Menu", "View Standings", "View Fixtures", "View Statistics"]
        self.selected_index = 0
        self.leagues = []
        self.selected_league = None

    def display_leagues(self, leagues):
        self.leagues = leagues
        self.screen.fill((0, 0, 0))  # Clear screen with black
        y_offset = 50

        # Display League List
        title = self.font.render("Leagues", True, (255, 255, 255))
        ...
        for i, league in enumerate(self.leagues):
            ...
            league_info = f"{league.id}: {league.name} - {league.season}"
            text = self.font.render(league_info, True, color)
            text_rect = text.get_rect(center=(400, y_offset + i * 30))
            self.screen.blit(text, text_rect)
        y_offset += len(self.leagues) * 30 + 20
